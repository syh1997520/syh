...
<span class="submenu"><a href="add_product.jsp" class="btn button_btn bg-deep-blue" title="添加产品"><i class="fa  fa-edit"></i>&nbsp;添加产品</a></span>
...
//页面加载，到servlet中获取数据
	$(function(){
		...
		$.get("shopServlet?op=query&stoId="+getQueryVariable('stoId'),function(data,status,xhr){
			//AJAX数据处理
	...
	$(function(){
		...
		$(document).on("click","#aprivPage",function(){
			...
			$.get("shopServlet?op=query&stoId="+getQueryVariable('stoId'),param,function(data,status,xhr){
				//AJAX数据处理
				getPageData(data,status,xhr);
