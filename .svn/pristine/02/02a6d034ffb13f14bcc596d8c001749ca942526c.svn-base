...
<script>
    ...
    $(function(){
           $.get("../front/userInfoServlet",{"r":"queryUnSpeakAble"},function(data,status,xhr){
...
//上一页时间
    ...
    $(document).on("click","#aprivPage",function(){
       ...
       var param={"r":"queryUnSpeakAble","currentPage":privPage,"pageSize":$("#pageSize").val()};
...
//下一页事件
   $(function(){
      ...
      if(nextPage>$("#allPage").val()){
         ...
         var param={"r":"queryUnSpeakAble","currentPage":nextPage,"pageSize":$("#pageSize").val()};
         $.get("../front/userInfoServlet",param,function(data,status,xhr){
         getPageData(data,status,xhr);
         });
      });
...
$(document).on("click","#acurrentPage",function(){
       var param={"r":"queryUnSpeakAble","currentPage":$(this).html(),"pageSize":$("#pageSize").val()};
       $.get("../front/userInfoServlet",param,function(data,status,xhr){
       getPageData(data,status,xhr);
       });
...
//页面容量改变
	...
	$(function(){
		...
		$(document).on("change","#pageSize",function(){
			...
			$.get("../front/userInfoServlet?r=queryUnSpeakAble",param,function(data,status,xhr){
				//AJAX数据处理
				getPageData(data,status,xhr);
    ...
    obj+='<td><img src="../front/'+element.userimg+'" style="width: 40px;height: 40px;"></td>';
     obj+='<td class="td-status"><span class="label label-success label-sm">已停用</span></td>';
     obj+='<td class="td-manage"><a title="启用" data-id="'+element.userId+'" onclick="return confirm(\'是否解除禁言？\')" href="javascript:;" class="btn button_btn btn-gray" id="speakAble">解除禁言</a>';
               
        obj+='<a title="删除"  onclick="return confirm(\'是否删除？\')" href="adminServlet?op=delUser2&userId='+element.userId+'"  class="btn button_btn btn-danger">删除</a>';
...
$(function(){
	...
	window.location.href="adminServlet?op=modUnSpeakAble&userId="+$(this).attr("data-id");
