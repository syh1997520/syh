...
public class AdminServlet extends HttpServlet {
  ...
  UserInfoService uSer=new UserInfoService();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		}else if(op.equals("delBus")){
			this.delBus(request, response);
		}else if(op.equals("delUser")){
			this.delUser(request, response);
		}else if(op.equals("delUser2")){
			this.delUser2(request, response);
		}
	 ...
	 * @throws IOException if an error occurred
	 */
	public void delUser2(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
	...
	public void delBus(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String strbusId=request.getParameter("busId");
		int busId=Integer.parseInt(strbusId);
		if(bSer.deleteBusiness(busId)){
			System.out.println("成功");
		}else{
			System.out.println("失败");
		}
		response.sendRedirect("adminbus.jsp");
