...
$(document).on("change","#pageSize",function(){
			...
			$.get("shopServlet?op=queryAllOrd",param,function(data,status,xhr){
				//AJAX数据处理
...
$(function(){
   $.get("shopServlet?op=queryAllOrd",function(data,status,xhr){
			//AJAX数据处理
			getPageData(data,status,xhr);
...
function getPageData(data,status,xhr){
         ...
         $.each($(oList),function(index,element){
              	var obj='<tr>';
              		...
              		obj+='<td>'+element.userInfo.userName+'</td>';
              		obj+='<td>'+element.ordPrice+'</td>';
              		obj+='<td>'+element.ordTime+'</td>';
              		obj+='<td>'+element.payTime+'</td>';
              		obj+='<td>'+element.ordStatic+'</td>';
              		obj+='<td>'+element.receiveAddress.recAddress+'</td>';
              		
              		
              		obj+='<td><a onclick="return confirm(\'是否删除？\')" href="shopServlet?op=delOrd&ordId='+element.ordId+'" class="btn button_btn btn-danger" id="del"><i class="fa fa-trash-o"></i>&nbsp;删除</a><a href="shopServlet?op=getOrd&ordId='+element.ordId+'" class="btn button_btn bg-deep-blue"><i class="fa fa-trash-o"></i>&nbsp;查看详情</a></td>';
              		obj+='</tr>';
              	$("#tBody").append(obj);
              });
             $("#ulpage").empty();
