...
//页面加载，到servlet中获取数据
	$(function(){
		...
		$.get("shopServlet?op=queryAll",function(data,status,xhr){
			//AJAX数据处理
	...
	$(function(){
		...
		$(document).on("click","#aprivPage",function(){
			...
			$.get("shopServlet?op=queryAll",param,function(data,status,xhr){
				//AJAX数据处理
				getPageData(data,status,xhr);
         ...
         //添加tbody中的tr
              $.each($(proList),function(index,element){
              	var obj='<tr>';
              		...
              		obj+='<td><a onclick="return confirm(\'是否删除？\')" href="adminServlet?op=del&proId='+element.proId+'" class="btn button_btn btn-danger" id="del"><i class="fa fa-trash-o"></i>&nbsp;删除</a><a class="btn button_btn bg-deep-blue" href="adminServlet?op=queryOne&proId='+element.proId+'"><i class="fa fa-trash-o"></i>&nbsp;修改</a></td>';
              		obj+='</tr>';
              	$("#tBody").append(obj);
              });
         //清空ul下的li
