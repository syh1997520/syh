...
$(document).on("change","#pageSize",function(){
			...
			$.get("businessServlet?op=queryBus",param,function(data,status,xhr){
				//AJAX数据处理
...
$(function(){
   $.get("businessServlet?op=queryBus",function(data,status,xhr){
			//AJAX数据处理
			getPageData(data,status,xhr);
...
function getPageData(data,status,xhr){
         ...
         $.each($(oList),function(index,element){
              	var obj='<tr>';
              		...
              		obj+='<td>'+element.busId+'</td>';
              		obj+='<td>'+element.busName+'</td>';
              		obj+='<td>'+element.busLoginName+'</td>';
              		obj+='<td>'+element.busPassword+'</td>';
    
              		obj+='<td><a onclick="return confirm(\'是否删除？\')" href="adminServlet?op=delBus&busId='+element.busId+'" class="btn button_btn btn-danger" id="del"><i class="fa fa-trash-o"></i>&nbsp;删除</a></td>';
              		obj+='</tr>';
              	$("#tBody").append(obj);
              });
             $("#ulpage").empty();
